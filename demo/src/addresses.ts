// Auto-generated by demo/start.sh — do not edit manually

export type FacilityKey = "senior" | "mezzanine" | "structured";

export const TOKEN_ADDRESS = "0x5FbDB2315678afecb367f032d93F642f64180aa3" as `0x${string}`;

export const FACILITIES: Record<FacilityKey, `0x${string}`> = {
  senior: "0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9" as `0x${string}`,
  mezzanine: "0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9" as `0x${string}`,
  structured: "0x5FC8d32690cc91D4c39d9d3abcBD16989F875707" as `0x${string}`,
};

export const FACILITY_KEYS: FacilityKey[] = ["senior", "mezzanine", "structured"];
